...
export const HttpStepConfigSchema = z
  .object({
...
export const ConditionalStepConfigSchema = z
  .object({
    expr: z.string().min(1),
  })
  .strict();
...
export function validateStepConfig(
  ...
  switch (type) {
    case 'HTTP':
      return HttpStepConfigSchema.safeParse(config);
    case 'SCRIPT':
      return ScriptStepConfigSchema.safeParse(config);
    case 'DELAY':
      return DelayStepConfigSchema.safeParse(config);
    case 'CONDITIONAL':
      return ConditionalStepConfigSchema.safeParse(config);
  }
}
